...
export default function App() {
  ...
  const glassBg = useTransform(scrollYProgress, [0, 0.1], ["rgba(245,245,245,0.4)", "rgba(245,245,245,0.8)"]);
  ...
  return (
    <div className="bg-[#F5F5F5] text-[#1A1A1A] font-sans selection:bg-[#C8A24A] selection:text-white overflow-x-hidden">
      <style>{`
        ...
        /* Steady Parallax - Image and Text fixed while page scrolls */
        ...
        .glass-nav { backdrop-filter: blur(40px); -webkit-backdrop-filter: blur(40px); border: 1px solid rgba(255,255,255,0.2); }
      ...
      {/* INDEPENDENT LOGO (DETACHED, 180px, ALWAYS VISIBLE) */}
      ...
      <div className="fixed top-8 right-8 md:top-12 md:right-12 z-[110]">
        <motion.div style={{ background: glassBg }} className="glass-nav px-10 py-5 rounded-full flex items-center gap-10 shadow-2xl">
          ...
          <a href="#contact" className="px-8 py-3 bg-[#C8A24A] text-[#1A1A1A] text-[10px] font-black tracking-widest uppercase hover:bg-[#1A1A1A] hover:text-white transition-all shadow-xl">PORTAL</a>
        </motion.div>
      </div>
      ...
      <div className="reveal-footer-container">
        ...
        {/* 1. HERO - AUDITED FOR 100% VISIBLE BG & ORIGINAL BRANDING */}
            ...
            <div className="lg:col-span-5 flex justify-center relative">
               <div className="relative w-[400px] h-[400px] md:w-[600px] md:h-[600px]">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                     <div className="w-64 h-64 md:w-[380px] md:h-[380px] bg-white border border-black/5 rounded-full flex flex-col items-center justify-center shadow-[0_40px_100px_rgba(0,0,0,0.06)] overflow-hidden">
                        <div className="text-center">
                           <div className="w-16 h-16 border-2 border-[#1A1A1A] mx-auto mb-6 flex items-center justify-center bg-white gold-glow relative">
                              <div className="w-4 h-4 bg-[#C8A24A] rounded-full" />
                           </div>
                           <h2 className="font-serif-display text-7xl uppercase leading-none text-[#1A1A1A] tracking-tighter">Square</h2>
                           <div className="text-[11px] tracking-[0.6em] text-[#C8A24A] font-black uppercase mt-1">CONNECT</div>
                        </div>
                     </div>
                  </div>
               </div>
        ...
        <section className="parallax-window">
           <div className="parallax-fixed-content">
              ...
              <div className="relative z-10 text-center px-6">
                <Reveal y={60}>
                  <h2 className="font-serif-display text-5xl md:text-[140px] tracking-tighter uppercase font-bold text-white leading-none text-center">FROM FRAGMENTATION<br />TO STRUCTURE.</h2>
                  <div className="mt-12 text-[#C8A24A] text-[11px] tracking-[2em] font-black uppercase text-center">MASTER ARCHITECTURE</div>
                </Reveal>
              </div>
           </div>
        </section>
        ...
        {/* 3. PROBLEM SECTION */}
        <section className="py-32 lg:py-64 bg-[#F5F5F5] relative z-10 shadow-2xl border-y border-black/5 text-left">
          <div className="container mx-auto max-w-7xl px-6 lg:px-12">
            <div className="grid lg:grid-cols-2 gap-24 items-end mb-40 reveal text-left">
              <div className="text-left">
                <div className="text-[11px] tracking-[0.4em] font-black text-[#1A1A1A]/30 mb-6 uppercase text-left">THE PROBLEM</div>
                <h2 className="font-serif-display text-7xl md:text-9xl tracking-tight leading-[0.85] text-[#1A1A1A] text-left">It's hard to do<br />it yourself.</h2>
              </div>
              <div className="max-w-xl text-left"><p className="text-[#1A1A1A]/50 text-2xl font-light leading-relaxed border-l-2 border-[#C8A24A]/40 pl-12 italic text-left">Managing six different specialists on your own is exhausting. The traditional system is fundamentally broken.</p></div>
            </div>
            <div className="grid md:grid-cols-3 gap-12 text-left">
              {["TOO MANY MEETINGS", "NO COMMUNICATION", "UNNECESSARY RISK"].map((t, idx) => (
                <div key={idx} className="relative p-16 bg-white border border-[#1A1A1A]/5 hover:border-[#C8A24A]/40 transition-all duration-700 group overflow-hidden shadow-sm hover:shadow-2xl text-left">
                  <div className="absolute top-4 left-6 font-serif-display text-[140px] text-[#1A1A1A]/5 italic leading-none pointer-events-none text-left">0{idx+1}</div>
        ...
        {/* 4. THE LIVE BRIDGE / TRANSFORMATION (SLOW LINEAR PULSE) */}
        <section id="transformation" className="min-h-[180vh] py-32 lg:py-56 bg-white relative overflow-hidden flex flex-col justify-center text-left">
          <div className="absolute inset-0 z-0">
             <img src="transformation-bg.png" alt="" className="w-full h-full object-cover opacity-[0.25] mix-blend-multiply" />
          </div>
          <div className="container mx-auto max-w-7xl px-6 lg:px-12 relative z-10 text-left">
            ...
            <div className="grid lg:grid-cols-12 gap-20 items-start text-left">
              <div className="lg:col-span-7 bg-[#F5F5F5]/40 backdrop-blur-sm border-2 border-[#1A1A1A]/5 p-8 md:p-24 shadow-2xl relative flex items-center justify-center h-[600px] md:h-[900px] text-left">
              ...
              <div className="lg:col-span-5 flex flex-col gap-8 pt-10 text-ink text-left">
                {engines.map((e, i) => (
                  <div key={i} className="p-12 border border-[#1A1A1A]/5 bg-white transition-all hover:border-[#C8A24A] hover:translate-x-6 shadow-sm group text-left">
        ...
        <section id="methodology" className="py-24 lg:py-56 bg-[#F5F5F5] border-b border-[#1A1A1A]/5 relative z-10 shadow-2xl text-left">
          <div className="container mx-auto max-w-7xl px-6 lg:px-12 text-left">
            <div className="mb-24 lg:mb-40 reveal text-left">
               <span className="text-[11px] tracking-[0.6em] uppercase text-[#C8A24A] font-black mb-8 block uppercase text-left">OUR METHODOLOGY</span>
               <h2 className="font-serif-display text-7xl md:text-8xl lg:text-[130px] leading-[0.8] tracking-tighter mb-10 text-[#1A1A1A] text-left">Architecting Growth.</h2>
               <p className="text-[#1A1A1A]/50 text-2xl font-light leading-relaxed max-w-3xl text-left">A meticulous 3-step process to total clarity and absolute speed.</p>
            </div>
            <div className="grid md:grid-cols-3 gap-16 lg:gap-24 text-left">
              {[
                ...
                <div key={i} className="reveal group text-left" style={{ transitionDelay: `${i * 200}ms` }}>
                   <div className="text-[14px] font-black tracking-[0.3em] text-[#C8A24A] mb-10 block transition-transform group-hover:translate-x-3 duration-700 text-left">0{step.id}</div>
                   <h3 className="text-[16px] font-black tracking-[0.4em] uppercase mb-10 text-[#1A1A1A] group-hover:text-[#C8A24A] transition-colors duration-700 text-left">{step.t}</h3>
        ...
        <section className="parallax-window">
           <div className="parallax-fixed-content">
              ...
              <div className="relative z-10 text-center px-6">
                <Reveal y={60}>
                  <h2 className="font-serif-display text-5xl md:text-[140px] tracking-tighter uppercase font-bold text-white leading-none text-center">BEYOND FRAGMENTATION<br /><span className="text-[#C8A24A] italic font-light lowercase capitalize leading-none opacity-80 text-center text-7xl md:text-[100px]">Unified Execution.</span></h2>
                </Reveal>
              </div>
           </div>
        </section>
        ...
        <section id="system" className="min-h-[150vh] py-32 lg:py-72 bg-white relative overflow-hidden flex flex-col justify-center z-10 shadow-2xl text-left">
          <div className="container mx-auto max-w-7xl px-6 lg:px-12 relative z-10 text-ink text-center max-w-4xl mx-auto mb-32">
            ...
            <p className="text-[#1A1A1A]/40 text-xl md:text-2xl font-light leading-relaxed uppercase tracking-[0.4em] text-center text-left">Engineered for speed and absolute precision.</p>
          </div>
          <div className="container mx-auto max-w-7xl px-6 lg:px-12 space-y-48 lg:space-y-72 mt-32 relative z-10 text-left">
              {[
                ...
                <div key={idx} className={`grid md:grid-cols-2 gap-24 lg:gap-40 items-center reveal ${step.side === 'left' ? 'md:flex-row-reverse' : ''} text-left`}>
                  <div className={`${step.side === 'left' ? 'md:order-2' : ''} text-left`}>
                    <div className="font-serif-display text-[120px] text-[#C8A24A]/20 mb-4 leading-none text-left">0{step.id}</div>
                  ...
                  <div className={`w-full aspect-[4/3] bg-[#F5F5F5] border border-[#1A1A1A]/10 relative group overflow-hidden shadow-2xl ${step.side === 'left' ? 'md:order-1' : ''} text-left`}>
        ...
        <section className="py-44 bg-[#1A1A1A] relative overflow-hidden text-center border-b border-white/5">
          <div className="container mx-auto max-w-7xl px-6 lg:px-12 relative z-10 text-center">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16 max-w-6xl mx-auto text-center">
              {[
                ...
                <div key={i} className="text-center p-12 md:p-16 border border-white/10 bg-white/5 backdrop-blur-md rounded-none hover:border-[#C8A24A]/40 transition-all duration-700 group shadow-xl text-center">
                  <div className="flex justify-center mb-10 text-center"><s.i size={56} className="text-[#C8A24A]/40 group-hover:text-[#C8A24A] transition-colors duration-500" /></div>
                  ...
                  <p className="text-[12px] tracking-[0.5em] uppercase font-black text-white/30 group-hover:text-white/60 transition-colors duration-500 text-center">{s.l}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <Reveal x={-50}>
              <div className="relative w-full max-w-md aspect-[4/5] bg-[#1A1A1A] border border-white/5 p-12 flex flex-col justify-end shadow-3xl group overflow-hidden text-left">
                ...
                <div className="relative z-20 text-left">
                  <span className="text-[10px] tracking-[0.8em] uppercase text-[#C8A24A] font-black block mb-10 text-left text-ink">THE MASTER ARCHITECT</span>
              ...
              <motion.div key={i} className="flex flex-col group text-left">
                ...
                <div className="flex items-center gap-10 mt-20 ml-20 text-left">
                  <div className="relative w-28 h-28 rounded-full border-2 border-[#C8A24A]/30 overflow-hidden flex items-center justify-center bg-stone-100 group-hover:border-[#C8A24A] transition-all duration-500 shadow-2xl scale-110 text-left">
                    {/* ASSET AUDIT: client-4 is .gif, others .png */}
        ...
        <section id="guarantee" className="py-32 lg:py-72 bg-[#1A1A1A] border-y border-white/5 relative overflow-hidden text-white z-0 shadow-2xl text-center">
          <div className="container mx-auto max-w-7xl px-6 lg:px-12 relative z-10 text-center">
            ...
            <div className="max-w-7xl mx-auto border-y border-white/10 py-32 mt-16 text-center">
            ...
            <div className="mt-24 max-w-4xl mx-auto px-6 text-center">
               <p className="text-3xl md:text-4xl font-serif-display italic text-white/30 leading-relaxed font-light text-center">"We align our success directly with yours. Mastery-level architecture designed to protect and multiply your life's assets."</p>
            </div>
          </div>
        </section>
        ...
        <section id="faq" className="py-24 lg:py-56 bg-[#1A1A1A] border-t border-white/5 text-white text-left">
          <div className="container mx-auto max-w-4xl px-6 text-center mb-32 text-center">
          ...
          <div className="container mx-auto max-w-4xl px-6 space-y-4 text-left">
            {faqs.map((faq, i) => (
              <div key={i} className={`border border-white/10 transition-all duration-700 ${openFaq === i ? 'bg-white/5 border-[#C8A24A]/40' : 'bg-transparent'} text-left`}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full p-12 flex justify-between items-center text-left hover:bg-white/5 transition-all text-left">
                ...
                <AnimatePresence>
                  {openFaq === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden text-left">
                      <p className="px-12 pb-12 text-lg text-white/40 leading-relaxed font-light italic border-l-2 border-[#C8A24A]/20 ml-12 text-left">{faq.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
        ...
        <section id="contact" className="py-32 lg:py-[40vh] bg-[#F5F5F5] text-center border-b border-[#1A1A1A]/10 shadow-[0_100px_200px_rgba(0,0,0,0.6)] relative overflow-hidden text-center">
          ...
          <div className="container mx-auto max-w-6xl px-6 relative z-10 text-center">
            <Reveal>
              <span className="text-[12px] tracking-[2.5em] text-[#C8A24A] font-black block mb-20 uppercase text-center">PRIVATE ACCESS</span>
              <h2 className="font-serif-display text-8xl lg:text-[220px] uppercase mb-20 font-bold leading-none tracking-tighter text-[#1A1A1A] text-center">Final Step.</h2>
              <p className="text-4xl lg:text-7xl font-serif-display text-[#C8A24A] mb-40 italic font-light leading-snug text-center text-ink">Ready to build your bridge?<br />Secure your unified future now.</p>
              <form className="flex flex-col sm:flex-row gap-8 max-w-5xl mx-auto mb-20 text-center" onSubmit={(e) => e.preventDefault()}>
                <input type="email" placeholder="YOUR EMAIL ADDRESS" required className="flex-1 bg-white border-2 border-black/10 px-16 py-10 text-lg font-bold tracking-widest outline-none focus:border-[#C8A24A] transition-all rounded-none text-ink" />
                <button type="submit" className="bg-[#C8A24A] text-[#1A1A1A] px-24 py-10 text-[14px] font-black tracking-[0.6em] uppercase hover:bg-black hover:text-white transition-all duration-700 shadow-3xl text-center">REQUEST ACCESS</button>
              </form>
              <p className="text-[#1A1A1A]/30 text-[12px] tracking-[1.5em] font-black uppercase text-center">CONFIDENTIALITY ASSURED • ADVISORY COVERAGE</p>
            </Reveal>
          </div>
        </section>
      </div>
      ...
      <footer className="sticky-footer py-32 bg-[#1A1A1A] text-white text-left">
        <div className="container mx-auto max-w-7xl px-6 lg:px-12 grid md:grid-cols-4 gap-24 text-white/90 relative z-10 text-left">
          <div className="col-span-1 text-left">
            <div className="w-32 h-32 mb-16 text-left">
